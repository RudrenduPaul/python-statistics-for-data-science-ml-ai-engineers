...
<script type="application/ld+json">
{
  ...
  "mainEntity": [
    ...
    {"@type": "Question", "name": "How does robust regression handle outliers differently than ordinary least squares?", "acceptedAnswer": {"@type": "Answer", "text": "Ordinary least squares minimizes squared error, so one extreme point can pull the fitted line far from the rest of the data. Chapter 9 covers robust Bayesian regression with a Student-t likelihood, whose heavier tails keep an outlier from dominating the fit."}},
    {"@type": "Question", "name": "What is a hierarchical (multilevel) Bayesian model?", "acceptedAnswer": {"@type": "Answer", "text": "A model that fits a per-group estimate for every group while letting each borrow statistical strength from a shared population distribution, so a sparsely observed group shrinks toward the population's typical behavior instead of reporting a noisy estimate built from almost nothing. Chapter 9 and Chapter 14 cover this."}},
    {"@type": "Question", "name": "Can a Gaussian process be used for classification, not just regression?", "acceptedAnswer": {"@type": "Answer", "text": "Yes. Chapter 11 covers Gaussian process classification, where a latent function passes through a sigmoid link into a Bernoulli likelihood, producing a flexible decision boundary a plain logistic regression cannot represent."}},
    {"@type": "Question", "name": "What is quantile regression, and how does it differ from predicting the mean?", "acceptedAnswer": {"@type": "Answer", "text": "Quantile regression predicts a specific quantile of an outcome's distribution, such as the 95th percentile, instead of its mean, catching tail behavior a mean-based model would miss. Chapter 13 covers this alongside conformal prediction, a distribution-free method for a prediction interval with a coverage guarantee."}},
    {"@type": "Question", "name": "What is a sample ratio mismatch (SRM) in A/B testing?", "acceptedAnswer": {"@type": "Answer", "text": "A gap between an A/B test's intended and observed traffic split, checked with a chi-square goodness-of-fit test; an undetected SRM invalidates the test's conclusion regardless of how the outcome comparison came out. Chapter 14 covers this."}},
    {"@type": "Question", "name": "Can an A/B test look like a win on average while harming a subgroup of users?", "acceptedAnswer": {"@type": "Answer", "text": "Yes. Chapter 14 covers a latent (unobserved) subgroup heterogeneity example, where a two-component mixture model surfaces a harmed subgroup that a single pooled analysis would miss."}}
  ]
}
</script>
```
